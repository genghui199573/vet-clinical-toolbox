...
(function(){
  ...
  function speciesSafety(p=state){
    ...
    if(/边牧|柯基|喜乐蒂|苏牧|澳牧|澳大利亚牧羊|柯利|古老英国牧羊犬|collie|corgi|sheltie|australian shepherd|old english sheepdog|border collie/i.test(b)) f.push({level:'WARN',code:'MDR1',message:'疑似 MDR1/ABCB1 高风险品种；使用大环内酯类/伊维菌素等前核对基因型、适应证和剂量。'});
    return f;
  }
  ...
  function rules(){return {
    ...
    md1Breeds:['Border Collie','Corgi','Shetland Sheepdog','Australian Shepherd','Collie'],
  ...
  // 5.0 Differential Diagnosis Atlas is additive. Load it after the DOM is available.
  function loadDDX(){
    if(document.getElementById('vet5-ddx-atlas-script')||window.__VET5_DDX_ATLAS__)return;
    const s=document.createElement('script');s.id='vet5-ddx-atlas-script';s.src='differential_diagnosis_5.0.js';s.defer=true;
    s.onerror=()=>console.warn('Vet Clinical Toolbox 5.0: Differential Diagnosis Atlas failed to load');
    document.head.appendChild(s);
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',loadDDX,{once:true});else loadDDX();
})();
