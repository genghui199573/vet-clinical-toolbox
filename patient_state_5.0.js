...
(function(){
  ...
  function injectUI(){
    ...
    if(header&&!$("globalPatientStrip")){const strip=document.createElement("div");strip.id="globalPatientStrip";strip.innerHTML=`<span>🧠 全局患者</span><b id="globalPatientHeaderBadge"></b><button type="button" id="globalPatientGo">患者状态</button>`;header.appendChild(strip);strip.style.cssText="display:flex;align-items:center;gap:8px;margin-top:7px;font-size:12px;flex-wrap:wrap";$("globalPatientGo").onclick=()=>{const h=$("home");if(h){document.querySelectorAll(".view").forEach(x=>x.classList.remove("active"));h.classList.add("active");document.querySelectorAll("nav button").forEach(b=>b.classList.toggle("active",b.dataset.v==="home"));location.hash="home"}}}
