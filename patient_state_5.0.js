...
(function(){
  ...
  const fields=["patientId","patientSpecies","patientBreed","patientAge","patientSex","patientWeight","patientChief","patientHistory","patientConditions","patientCreatinine","patientK","patientNa","patientAlb","patientGlucose","patientMeds","patientDx","patientOwner","patientPhone","patientPetName"];
  const aliases={
    ...
    patientChief:["caseChief","aiChief","cwRChief"], patientHistory:["caseHistory","aiHistory"], patientConditions:["casePast","aiQuestion"], patientDx:["caseDx"], patientMeds:["casePast","aiQuestion"], patientOwner:["caseOwner"], patientPhone:["casePhone"], patientPetName:["casePetName"],
    patientCreatinine:["cwCr"], patientK:["cwKTarget","qK","shK"], patientNa:["qSNa"], patientAlb:["qAlb"], patientGlucose:["erGlu","osGlu","ceGlu","r08DKAG"]
  };
  const state={version:"5.0-r08",updated_at:null,patientId:"",species:"犬",breed:"",age:"",sex:"",weight:null,chief:"",history:"",conditions:"",creatinine:null,potassium:null,sodium:null,albumin:null,glucose:null,currentMeds:"",diagnosis:"",ownerName:"",ownerPhone:"",petName:"",notes:"",labAnalyzerId:"",labAnalyzerLabel:""};
  ...
  function api(){
    ...
    ["version","updated_at","patientId","species","breed","age","sex","weight","chief","history","conditions","creatinine","potassium","sodium","albumin","glucose","currentMeds","diagnosis","ownerName","ownerPhone","petName","notes","labAnalyzerId","labAnalyzerLabel"].forEach(k=>Object.defineProperty(api,k,{enumerable:true,get:()=>state[k],set:v=>{state[k]=v}}));
    return api;
  }
  function persist(){
  ...
  function push(){
    set("patientId",state.patientId);set("patientSpecies",state.species);set("patientBreed",state.breed);set("patientAge",state.age);set("patientSex",state.sex);set("patientWeight",state.weight??"");set("patientChief",state.chief);set("patientHistory",state.history);set("patientConditions",state.conditions);set("patientCreatinine",state.creatinine??"");set("patientK",state.potassium??"");set("patientNa",state.sodium??"");set("patientAlb",state.albumin??"");set("patientGlucose",state.glucose??"");set("patientMeds",state.currentMeds);set("patientDx",state.diagnosis);set("patientOwner",state.ownerName);set("patientPhone",state.ownerPhone);set("patientPetName",state.petName);
  ...
  function summary(){return {患者编号:state.patientId||"未记录",物种:state.species,品种:state.breed||"未记录",年龄:state.age||"未记录",性别:state.sex||"未记录",体重:state.weight?state.weight+" kg":"未记录",主诉:state.chief||"未记录","既往/基础病":state.conditions||"未记录",肌酐:state.creatinine??"未记录",K:state.potassium??"未记录",Na:state.sodium??"未记录",白蛋白:state.albumin??"未记录",血糖:state.glucose??"未记录",当前用药:state.currentMeds||"未记录",诊断:state.diagnosis||"未记录",主人:state.ownerName||"未记录",手机号:state.ownerPhone||"未记录",宠物姓名:state.petName||"未记录",当前检验参考体系:state.labAnalyzerLabel||"未记录"};}
  function contextText(){return JSON.stringify(summary(),null,2)}
  function fillCase(){
    const map={patientId:"caseId",species:"caseSpecies",breed:"caseBreed",age:"caseAge",sex:"caseSex",weight:"caseWeight",chief:"caseChief",history:"caseHistory",conditions:"casePast",diagnosis:"caseDx",ownerName:"caseOwner",ownerPhone:"casePhone",petName:"casePetName"};Object.entries(map).forEach(([a,b])=>set(b,state[a]??""));
  ...
  function injectUI(){
    ...
    <div class="grid patient-core-grid">
      <div><label>患者/病例编号<input id="patientId" placeholder="留空自动生成"></label><label>宠物姓名<input id="patientPetName" placeholder="如 多多"></label><label>主人姓名<input id="patientOwner" placeholder="宠物主人"></label><label>主人手机号<input id="patientPhone" inputmode="tel" placeholder="11位手机号"></label><label>物种<select id="patientSpecies"><option>犬</option><option>猫</option><option>鹦鹉/鸟类</option><option>兔/啮齿类</option><option>爬宠/龟鳖</option><option>其他</option></select></label><label>品种<input id="patientBreed"></label><label>体重 kg<input id="patientWeight" type="number" step=".01"></label></div>
