...
function nav(id,label,after){
 const navEl=document.querySelector("#nav");
 if(!navEl)return;
 let b=navEl.querySelector(`[data-v="${id}"]`);
 if(!b){
   ...
   b.textContent=label;
   const a=navEl.querySelector(`[data-v="${after||"ai"}"]`);
   a?a.before(b):navEl.appendChild(b);
 }
...
function bindNav(){
 const navEl=$("#nav");
 if(!navEl)return;
 if(navEl.dataset.v5Bound)return;
 navEl.dataset.v5Bound="1";
...
async function addLabSection(){
 ...
 const b=document.createElement("button");b.type="button";b.dataset.v="labAnalyzer";b.setAttribute("aria-controls","labAnalyzer");b.textContent="检验仪器/参考区间";nav.appendChild(b);
 const sec=document.createElement("section");sec.id="labAnalyzer";sec.className="view";
 sec.innerHTML=`<div class="card"><h2>检验仪器 / 参考区间工作站</h2><div class="info">参考区间优先使用当前检验报告/本院验证区间，其次才是厂家资料。不同仪器、方法、试剂和实验室可存在差异。系统不猜测缺失范围。</div><div class="grid"><label>厂家/来源<select id="labMaker"></select></label><label>型号/配置<select id="labModel"></select></label><label>物种<select id="labSpecies"><option>犬</option><option>猫</option><option>其他</option></select></label><label>年龄组<select id="labAge"><option>成年</option><option>老年</option><option>幼年</option></select></label></div><div id="labMeta" class="result"></div><div class="toolbar"><input id="labQ" placeholder="检索项目，如 ALT / CREA / GLU / K / Na"><button class="primary" id="labActivate">设为当前参考体系</button><button class="primary" id="labRefresh">载入参考区间</button></div><div id="labActive" class="info">当前参考体系：未选择</div><div id="labTable" style="overflow:auto"></div><div class="warn">本院纳百资料：当前附件可核验的是成年猫报告区间；报告未显示具体仪器型号，因此系统标注为“纳百生化仪（型号待核对）”，不自动推算犬参考区间。</div></div>`;
 ...
 window.VCT50_LAB_REF=window.VCT50_LAB_REF||{};window.VCT50_LAB_REF.data=data;window.VCT50_LAB_REF.active=null;window.VCT50_LAB_REF.get=function(analyte,species){const x=this.active?.analyzer; if(!x)return null; const sp=species||this.active.species; return (x.reference_intervals?.values||[]).find(r=>String(r.analyte).toUpperCase()===String(analyte).toUpperCase() && (!r.species||r.species===sp) && (!r.age_group||r.age_group==="all"||String(r.age_group).includes((this.active.age||"").toLowerCase())))||null}; window.VCT50_LAB_REF.compare=function(analyte,value,unit,species){const r=this.get(analyte,species);if(!r||value==null)return null;let v=Number(value);if(!Number.isFinite(v))return null;const u=String(unit||"").toLowerCase(),ru=String(r.unit||"").toLowerCase();const mw={glucose:180.16,glu:180.16,bun:28.01,urea:60.06,cre:113.12,crea:113.12,calcium:40.078,ca:40.078,phos:30.974,phosphorus:30.974,albumin:1,alb:1};function cv(x,from,to,key){if(from===to)return x;if(key==="GLU"||key==="GLUCOSE"){if(from.includes("mg/dl")&&to.includes("mmol/l"))return x*10/180.16;if(from.includes("mmol/l")&&to.includes("mg/dl"))return x*180.16/10}if(key==="ALB"&&from.includes("g/dl")&&to.includes("g/l"))return x*10;if(key==="CREA"&&from.includes("mg/dl")&&to.includes("umol/l"))return x*88.4;if(key==="BUN"&&from.includes("mg/dl")&&to.includes("mmol/l"))return x*0.357;if(key==="PHOS"&&from.includes("mg/dl")&&to.includes("mmol/l"))return x*0.3229;if(key==="CA"&&from.includes("mg/dl")&&to.includes("mmol/l"))return x*0.2495;return x}v=cv(v,u,ru,String(analyte).toUpperCase());return {value:v,ref:r,status:r.low!=null&&r.high!=null?(v<r.low?"low":v>r.high?"high":"normal"):"unknown"};};
 function setActive(){const x=(data.analyzers||[]).find(a=>a.id===xs.value);if(!x)return;window.VCT50_LAB_REF.active={analyzer:x,species:$("#labSpecies").value,age:$("#labAge").value,updated_at:new Date().toISOString()};localStorage.setItem("vct50_active_lab_ref",JSON.stringify({id:x.id,species:$("#labSpecies").value,age:$("#labAge").value}));$("#labActive").innerHTML=`<b>当前参考体系：</b>${esc(x.manufacturer)} · ${esc(x.model)} · ${esc($("#labSpecies").value)} · ${esc($("#labAge").value)}；后续实验室模式引擎优先读取此体系。`;window.dispatchEvent(new CustomEvent("vct50:lab-ref-change",{detail:window.VCT50_LAB_REF.active}));render();}
 function render(){const x=(data.analyzers||[]).find(a=>a.id===xs.value);if(!x)return;const st=x.reference_intervals?.status;$("#labMeta").innerHTML=`<b>${esc(x.manufacturer)} · ${esc(x.model)}</b><br>${esc(x.origin||"")}<br>检测：${esc((x.tests||[]).join("、"))}<br>${st==="official"?"<span class='pill'>官方参考区间</span>":st==="hospital_validated_report"?"<span class='pill'>本院报告验证区间</span>":"<span class='pill'>未核验厂家区间</span>"}${x.source?.note?`<br><span class='small'>${esc(x.source.note)}</span>`:""}`;let rows=x.reference_intervals?.values||[],q=($("#labQ").value||"").trim().toUpperCase(),sp=$("#labSpecies").value,age=$("#labAge").value.toLowerCase();rows=rows.filter(r=>(!q||String(r.analyte).toUpperCase().includes(q))&&(!r.species||r.species===sp)&&(!r.age_group||r.age_group==="all"||String(r.age_group).includes(age)));$("#labTable").innerHTML=rows.length?`<table><thead><tr><th>项目</th><th>单位</th><th>下限</th><th>上限</th><th>备注</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.analyte)}</td><td>${esc(r.unit)}</td><td>${r.low??""}</td><td>${r.high??""}</td><td>${esc(r.note||"")}</td></tr>`).join("")}</tbody></table>`:"<div class='muted'>该参考体系/物种目前没有可核验区间。</div>"}
 ...
 ms.onchange=models;xs.onchange=render;$("#labActivate").onclick=setActive;$("#labRefresh").onclick=render;$("#labQ").oninput=render;$("#labSpecies").onchange=render;$("#labAge").onchange=render;
 const saved=JSON.parse(localStorage.getItem("vct50_active_lab_ref")||"null"); if(saved?.id){const found=(data.analyzers||[]).find(a=>a.id===saved.id);if(found){ms.value=found.manufacturer;models();xs.value=found.id;$("#labSpecies").value=saved.species||"猫";$("#labAge").value=saved.age||"成年";setActive()}} else {const local=(data.analyzers||[]).find(a=>a.id==="dingdang-nabai-biochemistry");if(local){ms.value=local.manufacturer;models();xs.value=local.id;$("#labSpecies").value="猫";$("#labAge").value="成年";setActive()}}
}
...
function main(){bindNav();addLabSection();addPWA()}
